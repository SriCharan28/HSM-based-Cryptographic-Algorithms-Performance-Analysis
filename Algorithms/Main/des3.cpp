...
CK_BYTE IV[] = "12345678";  // DES3 uses 8-byte IV
...
double calculateThroughput(void (operation)(), const char* operationName, int numOperations);
...
// Function to generate DES3 key
void generateDes3Key()
{
    CK_MECHANISM mech = { CKM_DES3_KEY_GEN, nullptr, 0 };
...
// Function to calculate throughput for a given operation and return elapsed time
double calculateThroughput(void (operation)(), const char* operationName, int numOperations)
{
    ...
    double throughput = numOperations / elapsedTime.count();
    cout << operationName << " throughput: " << throughput << " operations per second" << endl;

    return elapsedTime.count(); // Return total elapsed time in seconds
}
...
int main(int argc, char **argv)
{
    ...
    // Measure throughput for key generation, encryption, and decryption
    int numOperations = 100;

    // Calculate key generation time
    double keyGenTime = calculateThroughput(generateDes3Key, "Key generation", numOperations);
    cout << "Key generation time: " << keyGenTime << " seconds for " << numOperations << " operations" << endl;

    double encTime = calculateThroughput(encryptData, "Encryption", numOperations);
    double decTime = calculateThroughput(decryptData, "Decryption", numOperations);
